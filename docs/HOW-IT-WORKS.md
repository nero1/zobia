...
**Screen coverage.** `apps/android/src/routes/` currently ports: home, games (list, detail, saved), rooms (list + detail), messages (list + conversation), moments (feed + create), Answers (list, ask, question detail — `answers/`), blogs (list, post, new), business (hub, pages, ads), ads, profile (own + `$username`), wallet, stats, settings, notifications, auth, and — as of v2.06 — Quests, Friends, Gifts, Events, Inbox, Elder, Referrals, Classroom, and Leaderboards. Not yet ported: Guilds, Guild Discovery, Council, Community Notes, Nemesis, Economy, Prestige, Seasons, Creator tools, Merch, single-Gift, Stickers, KYC, Message Groups, Room creation, Business/Subscription settings — see the PRD's v2.06 changelog for the full list. When porting one, follow the pattern in the existing route files: TanStack Router file route, `apiClient` + TanStack Query (never raw `fetch`), Tailwind classes matching the equivalent web/PWA screen, i18n keys added to `shared/i18n/locales/en.json`.
