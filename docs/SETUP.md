...
A placeholder `google-services.json` and the Google Services Gradle plugin (`com.google.gms:google-services`, applied in `apps/android/android/build.gradle` and `apps/android/android/app/build.gradle`) are already committed — same pattern as the Expo app (see "GitHub Actions auto-build" below). This is required even if you never configure real FCM sends: without a `google-services.json` on disk, `FirebaseMessaging.getInstance()` throws `IllegalStateException: Default FirebaseApp is not initialized` the moment `PushNotifications.register()` runs, which crashes the whole app (uncaught on the native `CapacitorPlugins` thread — the JS-side `try/catch` in `apps/android/src/lib/push/index.ts` can't catch a native-side crash). The placeholder is enough to let `FirebaseApp` initialize and avoid the crash; it can't actually deliver notifications.
...
6. Run the schema (the entire database, one file — every table, constraint,
   ...
   `docs/HOW-IT-WORKS.md`), `0007_gift_message.sql` (the optional gift
   message — `gifts.message`/`message_word_count` plus the
   `gift_message_*` x_manifest keys — see "Gift Messages" in the PRD §12),
   `0008_account_appeals.sql` (the
   ...
   `0009_admin_users_reports_index.sql` (missing index on
