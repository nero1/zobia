    ...
    <>
      ...
      <div className="fixed left-4 right-4 top-1/2 z-50 max-h-[85vh] -translate-y-1/2 overflow-y-auto rounded-2xl bg-white dark:bg-neutral-800 shadow-2xl">
        ...
        <div className="space-y-3 px-4 py-4">
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-neutral-700 dark:text-neutral-300">{t('creator.broadcasts.subjectLabel')}</label>
            <input
              value={subject}
              onChange={(e) => setSubject(e.target.value)}
              maxLength={200}
              placeholder={t('creator.broadcasts.subjectPlaceholder')}
              className="w-full rounded-xl border border-neutral-300 dark:border-neutral-600 px-3 py-2.5 text-sm focus:border-primary-500 focus:outline-none"
            />
          </div>
          <div>
            <div className="mb-1 flex items-center justify-between text-xs font-semibold text-neutral-700 dark:text-neutral-300">
              <span>{t('creator.broadcasts.bodyLabel')}</span>
              <span className={body.length > 950 ? 'text-red-500' : 'text-neutral-400 dark:text-neutral-500'}>{t('creator.broadcasts.charCount', { current: body.length })}</span>
            </div>
            <textarea
              value={body}
              ...
              className="w-full resize-none rounded-xl border border-neutral-300 dark:border-neutral-600 px-3 py-2.5 text-sm focus:border-primary-500 focus:outline-none"
            />
          </div>
          {localError && <p className="text-xs text-red-600 dark:text-red-300">{localError}</p>}
          <div className="flex gap-3">
            ...
            <button onClick={onClose} className="rounded-xl border border-neutral-300 dark:border-neutral-600 px-4 py-2.5 text-sm font-semibold text-neutral-700 dark:text-neutral-300">
...
function CreatorBroadcastsPage() {
  ...
  if (status === 'error' || !data) {
    return <div className="p-6 text-sm text-red-600 dark:text-red-300">{t('creator.broadcasts.loadError')}</div>;
  }

  return (
    <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800 space-y-3 px-4 py-4">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{t('creator.broadcasts.title')}</h1>
      ...
      <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4 shadow-card">
        <h2 className="mb-2 text-sm font-semibold text-neutral-700 dark:text-neutral-300">{t('creator.broadcasts.monthlyAllowance')}</h2>
        <div className="flex items-center justify-between">
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            <span className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{data.allowance.freeRemaining}</span>{' '}
          ...
          {!data.allowance.canSend && data.allowance.reason && (
            <span className="rounded-full bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 text-xs font-semibold text-neutral-600 dark:text-neutral-400">{data.allowance.reason}</span>
          )}
        </div>
        <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{t('creator.broadcasts.tierLabel', { tier: data.allowance.tier })}</p>
      </div>
      ...
      <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 shadow-card">
        <div className="border-b border-neutral-100 dark:border-neutral-800 px-4 py-3">
          <h2 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">{t('creator.broadcasts.historyTitle')}</h2>
        </div>
        {data.broadcasts.length === 0 ? (
          <div className="px-4 py-8 text-center text-sm text-neutral-500 dark:text-neutral-400">{t('creator.broadcasts.historyEmpty')}</div>
        ) : (
          <div className="divide-y divide-neutral-100 dark:divide-neutral-700">
            {data.broadcasts.map((b) => (
              <div key={b.id} className="px-4 py-3">
                <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{b.subject || t('creator.broadcasts.subjectOptional')}</h3>
                <p className="mt-1 line-clamp-2 text-sm text-neutral-600 dark:text-neutral-400">{b.body}</p>
