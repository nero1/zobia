  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white dark:bg-neutral-800">
      ...
      <div className="flex-1 overflow-y-auto space-y-3.5 p-4" style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}>
        ...
        {showActiveToggle && (
          <label className="flex items-center gap-2.5 text-sm font-medium text-neutral-700 dark:text-neutral-300">
            <input type="checkbox" checked={form.isActive} onChange={(e) => setForm((f) => ({ ...f, isActive: e.target.checked }))} className="h-4 w-4 rounded border-neutral-300 dark:border-neutral-600" />
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-neutral-800 p-5 shadow-xl">
        <h3 className="mb-1 text-base font-bold text-neutral-900 dark:text-neutral-100">{t('admin.brandedRooms.broadcastTitle', 'Broadcast — {{brand}}', { brand: room.brandName })}</h3>
        <p className="mb-3 text-xs text-neutral-500 dark:text-neutral-400">{t('admin.brandedRooms.broadcastHint', 'Send a sponsored message to this room. Coin bonus is deducted from the sponsor budget.')}</p>
  ...
  return (
    <div className="px-4 py-5">
      <div className="mb-1 flex items-center justify-between gap-2">
        <h1 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{t('admin.nav.brandedRooms', 'Branded Rooms')}</h1>
            ...
            return (
              <AdminCard key={room.id}>
                <div className="mb-1.5 flex items-center gap-2">
                  ...
                  <p className="font-semibold text-neutral-900 dark:text-neutral-100 truncate">{room.brandName}</p>
                ...
                <p className="mb-2.5 text-[10px] text-neutral-400 dark:text-neutral-500">{fmtDate(room.startsAt)} — {fmtDate(room.endsAt)}</p>

                <div className="flex flex-wrap gap-1.5">
                  <button type="button" onClick={() => setEditing(room)} className="rounded-lg bg-blue-100 dark:bg-blue-900/40 px-2.5 py-1 text-xs font-semibold text-blue-700 dark:text-blue-300">
                  ...
                  <button type="button" onClick={() => setBroadcasting(room)} className="rounded-lg bg-teal-100 dark:bg-teal-900/40 px-2.5 py-1 text-xs font-semibold text-teal-700 dark:text-teal-300">
                    {t('admin.brandedRooms.broadcast', 'Broadcast')}
                  </button>
                  <button type="button" onClick={() => setDeleteTarget(room)} className="rounded-lg bg-danger-100 dark:bg-danger-900/40 px-2.5 py-1 text-xs font-semibold text-danger-700 dark:text-danger-300">
