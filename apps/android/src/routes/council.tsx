...
const IDEA_STATUS: Record<string, string> = {
  open: 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400',
  under_review: 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300',
  accepted: 'bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300',
    ...
    <div className="rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1 text-xs text-neutral-500 dark:text-neutral-400">
          <Link to="/profile/$username" params={{ username: idea.authorUsername }} className="font-medium text-primary-600 dark:text-primary-300">
            @{idea.authorUsername}
          </Link>
          <span>·</span>
...
function CouncilPage() {
  ...
  return (
    <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800 px-4 py-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{t('council.title')}</h1>
        <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">{t('council.subtitle')}</p>
      </div>

      <section>
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">{t('council.membersSection')}</h2>
        <div className="rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 divide-y divide-neutral-100 dark:divide-neutral-700">
          {membersStatus === 'pending' ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="flex animate-pulse items-center gap-3 px-4 py-3">
                ...
                <div className="h-3 flex-1 rounded bg-neutral-200 dark:bg-neutral-700" />
              </div>
            ))
          ) : (members ?? []).length === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-neutral-500 dark:text-neutral-400">{t('council.noMembers')}</div>
          ) : (
            (members ?? []).map((m) => (
              <div key={m.userId} className="flex items-center gap-3 px-4 py-3">
                <span className="w-6 text-center text-sm font-bold text-neutral-400 dark:text-neutral-500">#{m.rank}</span>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">{t('council.ideasSection')}</h2>
        ...
        {showForm && (
          <form
            ...
            className="mb-4 rounded-2xl border border-primary-200 bg-primary-50 dark:bg-primary-900/30 p-4"
          >
            <h3 className="mb-3 font-semibold text-neutral-900 dark:text-neutral-100">{t('council.newIdea')}</h3>
            {formError && <p className="mb-3 text-sm text-danger-600 dark:text-danger-300">{formError}</p>}
            <input
              type="text"
              value={title}
              ...
              required
              maxLength={120}
              className="mb-3 w-full rounded-xl border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-4 py-2.5 text-sm focus:outline-none"
              data-selectable
            />
            <textarea
              value={description}
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowForm(false)} className="flex-1 rounded-xl border border-neutral-300 dark:border-neutral-600 py-2 text-sm font-semibold text-neutral-700 dark:text-neutral-300">
                {t('council.cancel')}
              </button>
