...
function QuizDetailPage() {
  ...
  const fundTreasury = useMutation({
    mutationFn: ({ amount, maxClaimants }: { amount: number; maxClaimants: number }) =>
      isEditingTreasury
        ? apiClient.patch<Treasury>(`/quizzes/${slug}/treasury`, { amount, maxClaimants })
        : apiClient.post<Treasury>(`/quizzes/${slug}/treasury`, { amount, maxClaimants }),
    onSuccess: (res) => {
  ...
  const closeTreasury = useMutation({
    mutationFn: () => apiClient.delete<Treasury>(`/quizzes/${slug}/treasury`),
    onSuccess: (res) => {
      qc.setQueryData(['quizzes', 'treasury', slug], res.data);
    },
    onError: (err) => {
  ...
  return (
    <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800 p-4 space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        {quiz.isOwner && isEditingTreasury && (
          <button
