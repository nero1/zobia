...
function QuizCard({ q }: { q: QuizSummary }) {
  ...
  return (
    <Link to="/quizzes/$slug" params={{ slug: q.slug }} className="block bg-white dark:bg-neutral-800 border-b border-neutral-100 dark:border-neutral-800 p-4">
      <h3 className="line-clamp-2 text-sm font-semibold text-neutral-900 dark:text-neutral-100">{q.title}</h3>
  ...
  if (featureFlags && featureFlags.quizzes === false) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2 bg-neutral-50 dark:bg-neutral-800 px-6 text-center">
        <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('quizzes.disabled', 'Quizzes are currently disabled.')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800">
      <div className="flex items-center justify-between bg-white dark:bg-neutral-800 px-4 py-3 border-b border-neutral-100 dark:border-neutral-800">
        <h1 className="text-lg font-bold text-neutral-900 dark:text-neutral-100">{t('quizzes.title', 'Quizzes')}</h1>
