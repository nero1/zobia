        ...
        {canPost ? (
          composerOpen ? (
            <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-3 space-y-2">
              <input value={title} onChange={(e) => setTitle(e.target.value.slice(0, 200))} placeholder={t('bbforum.thread.titleLabel')} className="w-full rounded-lg border border-neutral-300 dark:border-neutral-600 bg-neutral-50 dark:bg-neutral-800 px-3 py-2 text-sm" />
              <div className="flex gap-1 text-xs font-semibold">
                {(['plaintext', 'markdown'] as const).map((fmt) => (
                  <button key={fmt} onClick={() => setContentFormat(fmt)} className={`rounded-t-lg px-3 py-1.5 ${contentFormat === fmt ? 'bg-neutral-100 dark:bg-neutral-800 text-primary-700 dark:text-primary-300' : 'text-neutral-400 dark:text-neutral-500'}`}>
              ...
              {potEnabled && (
                <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-600 dark:text-neutral-400">
                  <input type="number" min={1} value={potPerClaim} onChange={(e) => setPotPerClaim(Math.max(1, parseInt(e.target.value, 10) || 1))} className="w-16 rounded border border-neutral-300 dark:border-neutral-600 px-2 py-1 text-center" />
                  <span>x</span>
                  <input type="number" min={1} value={potMaxClaims} onChange={(e) => setPotMaxClaims(Math.max(1, parseInt(e.target.value, 10) || 1))} className="w-16 rounded border border-neutral-300 dark:border-neutral-600 px-2 py-1 text-center" />
                  <span>= {potPerClaim * potMaxClaims} Credits charged now</span>
                </div>
              )}
              <div className="flex gap-2">
                <button onClick={() => setComposerOpen(false)} className="rounded-lg border border-neutral-300 dark:border-neutral-600 px-3 py-1.5 text-xs font-semibold text-neutral-700 dark:text-neutral-300">{t('bbforum.thread.cancel')}</button>
            ...
            <button onClick={() => setComposerOpen(true)} className="w-full rounded-xl border border-dashed border-neutral-300 dark:border-neutral-600 px-4 py-3 text-left text-sm text-neutral-500 dark:text-neutral-400">
              {t('bbforum.board.newThread')}
            </button>
          )
        ) : (
          <p className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-4 py-3 text-xs text-neutral-400 dark:text-neutral-500">
            {t('bbforum.levelTooLow', { level: bbforumConfig.minLevelToPost, current: myLevel ?? 1 })}
          </p>
        )}
      </div>

      {status === 'pending' && <div className="p-3"><div className="h-16 rounded bg-neutral-200 dark:bg-neutral-700 animate-pulse" /></div>}

      {status === 'success' && threads.length === 0 && (
        <p className="p-6 text-center text-sm text-neutral-500 dark:text-neutral-400">{t('bbforum.board.noThreads')}</p>
      )}

      <div className="divide-y divide-neutral-100 dark:divide-neutral-700 bg-white dark:bg-neutral-800">
        {threads.map((thread) => (
