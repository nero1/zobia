...
function HelpCategoryPage() {
  ...
  return (
    <div className="p-4">
      ...
      {data.category.description && <p className="mb-4 text-sm text-neutral-400 dark:text-neutral-500">{data.category.description}</p>}
      ...
      <div className="space-y-2">
        {data.docs.map((doc) => (
          <Link key={doc.id} to="/help/$category/$doc" params={{ category: data.category.slug, doc: doc.slug }} className="block rounded-xl border border-neutral-800 bg-neutral-900 p-3">
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400">{t(`help.difficulty.${doc.difficulty}`, doc.difficulty)}</p>
          </Link>
        ))}
      </div>
    </div>
