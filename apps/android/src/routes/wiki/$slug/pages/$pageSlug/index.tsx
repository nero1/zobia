...
function WikiPageViewPage() {
  ...
  if (!pageQuery.data) return <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800 p-6 text-center text-sm text-neutral-500 dark:text-neutral-400">{t('wiki.pages.notFound', 'Page not found.')}</div>;
    ...
    <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800 p-4 space-y-4">
      <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4">
        <div className="flex items-start justify-between gap-2">
          <h1 className="text-lg font-bold text-neutral-900 dark:text-neutral-100">{page.title}</h1>
          <Link to="/wiki/$slug" params={{ slug }} className="flex-shrink-0 text-xs text-neutral-400 dark:text-neutral-500 underline underline-offset-2">
        ...
        {page.content_format === 'plaintext' ? (
          <p className="mt-3 whitespace-pre-wrap text-sm text-neutral-800 dark:text-neutral-200">{page.content_markdown}</p>
        ) : (
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-neutral-100 dark:border-neutral-800 pt-3">
          {canContribute && (
            <Link to="/wiki/$slug/pages/$pageSlug/edit" params={{ slug, pageSlug }} className="rounded-lg bg-neutral-100 dark:bg-neutral-800 px-3 py-1.5 text-xs font-semibold text-neutral-700 dark:text-neutral-300">
              {t('wiki.pages.edit', 'Edit')}
            </Link>
          )}
          <Link to="/wiki/$slug/pages/$pageSlug/revisions" params={{ slug, pageSlug }} className="rounded-lg bg-neutral-100 dark:bg-neutral-800 px-3 py-1.5 text-xs font-semibold text-neutral-700 dark:text-neutral-300">
            {t('wiki.pages.viewHistory', 'History ({{count}})', { count: page.revision_count })}
          </Link>
          {canManage && !confirmingDelete && (
            <button onClick={() => setConfirmingDelete(true)} className="rounded-lg bg-red-100 dark:bg-red-900/40 px-3 py-1.5 text-xs font-semibold text-red-700 dark:text-red-300">
              {t('common.delete', 'Delete')}
            </button>
          )}
          {canManage && confirmingDelete && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-red-700 dark:text-red-300">{t('wiki.pages.confirmDelete', 'Delete this page?')}</span>
              ...
              <button onClick={() => setConfirmingDelete(false)} className="rounded-lg bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 text-xs font-semibold text-neutral-700 dark:text-neutral-300">
                {t('common.cancel')}
              </button>
            </div>
          )}
