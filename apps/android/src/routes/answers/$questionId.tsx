...
function QuestionDetailPage() {
  ...
  function AnswerNodeView({ node }: { node: AnswerNode }) {
    ...
    return (
      <div className={node.depth > 0 ? 'mt-2 border-l-2 border-neutral-200 dark:border-neutral-700 pl-2' : 'mt-2'} style={{ marginLeft: Math.min(node.depth, MAX_VISUAL_DEPTH) > 0 && node.depth === 0 ? 0 : undefined }}>
        <div className="rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-3">
          <div className="mb-1.5 flex flex-wrap items-center gap-2 text-xs text-neutral-500 dark:text-neutral-400">
          ...
          {replyingTo === node.id && (
            <div className="mt-3 space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => setReplyingTo(null)} className="rounded-lg border border-neutral-300 dark:border-neutral-600 px-3 py-1.5 text-xs font-semibold text-neutral-600 dark:text-neutral-400">{t('answers.ask.cancel')}</button>
                <button
        ...
        {showMoreCount > 0 && (
          <button onClick={() => handleExpandThread(node.id)} disabled={expandingId === node.id} className="mt-2 ml-2 text-xs font-semibold text-primary-600 dark:text-primary-300 disabled:opacity-50">
  ...
  if (!question) {
    return (
      <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800 p-6 text-center">
        <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('answers.detail.notFound')}</p>
        <Link to="/answers" className="mt-3 inline-block text-sm font-semibold text-primary-600 dark:text-primary-300">← {t('answers.title')}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800 p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1 text-xs">
          {(['best', 'new'] as const).map((s) => (
            <button key={s} onClick={() => setSort(s)} className={`rounded-lg px-2.5 py-1 font-semibold ${sort === s ? 'bg-neutral-900 text-white' : 'text-neutral-500 dark:text-neutral-400'}`}>
      ...
      {answersQuery.isPending ? (
        ...
        <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('answers.noAnswers')}</p>
      ) : (
      ...
      {bypassPrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setBypassPrompt(null)}>
          <div className="relative w-full max-w-sm rounded-2xl bg-white dark:bg-neutral-800 p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              <button onClick={() => setBypassPrompt(null)} className="flex-1 rounded-xl border border-neutral-300 dark:border-neutral-600 py-2 text-sm font-semibold text-neutral-700 dark:text-neutral-300">{t('answers.ask.cancel')}</button>
              <button
