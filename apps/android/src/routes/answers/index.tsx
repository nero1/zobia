  ...
  return (
    <div className="h-full overflow-y-auto bg-neutral-50 dark:bg-neutral-800">
      <div className="flex items-center justify-between bg-white dark:bg-neutral-800 px-4 py-3 border-b border-neutral-100 dark:border-neutral-800">
        <h1 className="text-lg font-bold text-neutral-900 dark:text-neutral-100">{t('answers.title')}</h1>
        {canPost ? (
          <Link to="/answers/ask" className="rounded-lg bg-primary-600 px-3 py-2 text-xs font-semibold text-white">
            + {t('answers.ask.cta')}
          </Link>
        ) : (
          <span className="rounded-lg border border-neutral-200 dark:border-neutral-700 px-3 py-2 text-xs font-semibold text-neutral-400 dark:text-neutral-500">
      ...
      {status === 'pending' && (
        <div>{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-20 border-b border-neutral-100 dark:border-neutral-800 bg-white dark:bg-neutral-800 p-4 animate-pulse" />)}</div>
      )}
