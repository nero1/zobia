...
const STAT_COLOR_MAP: Record<string, string> = {
  blue: 'border-blue-200 bg-blue-50 dark:bg-blue-900/30',
  green: 'border-teal-200 bg-teal-50 dark:bg-teal-900/30',
  gold: 'border-amber-200 bg-amber-50 dark:bg-amber-900/30',
  red: 'border-danger-200 bg-danger-50 dark:bg-danger-900/30',
  neutral: 'border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800',
};
...
export function AdminStatCard({
  ...
  return (
    <div className={`rounded-xl border p-4 shadow-card ${STAT_COLOR_MAP[color]}`}>
      <p className="text-[11px] font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">{label}</p>
      <p className="mt-1.5 text-xl font-bold text-neutral-900 dark:text-neutral-100">{value}</p>
...
export function AdminStatSkeleton() {
  return (
    <div className="animate-pulse rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4">
      ...
      <div className="mt-2.5 h-5 w-12 rounded bg-neutral-200 dark:bg-neutral-700" />
    </div>
  );
}
...
export function AdminSectionHeader({ children }: { children: ReactNode }) {
  return <h2 className="mb-2.5 text-[11px] font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">{children}</h2>;
}
...
const BADGE_COLOR_MAP: Record<string, string> = {
  ...
  red: 'bg-danger-100 dark:bg-danger-900/40 text-danger-700 dark:text-danger-300',
...
export function AdminCard({ children, onClick }: { children: ReactNode; onClick?: () => void }) {
  ...
  return (
    <Comp
      ...
      onClick={onClick}
      className={`w-full rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4 text-left shadow-card ${onClick ? 'active:bg-neutral-50 dark:active:bg-neutral-800' : ''}`}
    >
      {children}
    </Comp>
  );
...
export function AdminToggle({ checked, onChange, disabled }: { checked: boolean; onChange: (next: boolean) => void; disabled?: boolean }) {
  return (
    ...
    >
      <span
        className={`absolute top-0.5 h-5 w-5 rounded-full bg-white dark:bg-neutral-800 shadow transition-transform ${checked ? 'translate-x-[22px]' : 'translate-x-0.5'}`}
      />
    </button>
  );
}
      ...
      {tabs.map((tab) => (
        <button
          ...
          className={`shrink-0 rounded-full px-3.5 py-1.5 text-sm font-semibold transition-colors ${
            active === tab.key ? 'bg-neutral-900 text-white' : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400'
          }`}
        >
          {tab.label}
        </button>
...
export function AdminField({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-semibold text-neutral-600 dark:text-neutral-400">{label}</span>
      {children}
    </label>
  );
}
...
export const adminInputClass =
  'w-full rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3 py-2.5 text-sm text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-400 dark:text-neutral-500 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500';
