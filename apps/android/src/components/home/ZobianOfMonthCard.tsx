...
function Skeleton() {
  return (
    <div className="animate-pulse rounded-2xl border border-amber-200 bg-white dark:bg-neutral-800 p-5 shadow-sm">
...
export function ZobianOfMonthCard() {
  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 dark:bg-amber-900/40 px-2.5 py-0.5 text-xs font-bold text-amber-700 dark:text-amber-300">
      ...
      <div className="mt-3 flex items-center gap-4">
        <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full bg-amber-100 dark:bg-amber-900/40 text-3xl ring-2 ring-amber-300">
          {data.avatarUrl ? <img src={data.avatarUrl} alt="" className="h-full w-full object-cover" /> : data.avatarEmoji || '🌟'}
        </div>
        <div className="min-w-0">
          <p className="truncate text-base font-bold text-neutral-900 dark:text-neutral-100">{data.displayName}</p>
          <p className="truncate text-sm text-neutral-500 dark:text-neutral-400">@{data.username}</p>
          {data.score && <p className="mt-0.5 text-xs font-semibold text-amber-700 dark:text-amber-300">{t('home.zobianOfMonth.score', { score: data.score })}</p>}
        </div>
      </div>
      {data.note && <p className="mt-3 text-xs italic text-neutral-500 dark:text-neutral-400">&ldquo;{data.note}&rdquo;</p>}
    </Link>
  );
}
