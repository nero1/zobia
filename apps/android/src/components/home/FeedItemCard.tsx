...
export function FeedItemCard({ item }: { item: FeedItem }) {
  ...
  return (
    <Link
      ...
      className="flex gap-3 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm transition-colors active:bg-neutral-50"
    >
      {item.imageUrl ? (
        <img src={item.imageUrl} alt="" className="h-16 w-16 shrink-0 rounded-lg object-cover" />
      ) : (
        <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-neutral-100 text-2xl">
      ...
      <div className="min-w-0 flex-1">
        ...
        {item.title && <p className="truncate text-sm font-semibold text-neutral-900">{item.title}</p>}
        {item.excerpt && <p className="mt-0.5 line-clamp-2 text-xs text-neutral-500">{item.excerpt}</p>}
...
export function FeedItemCardSkeleton() {
  return (
    <div className="flex animate-pulse gap-3 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
