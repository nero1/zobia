...
export function BlogOwnerToolbar({ blogSlug, isDraft }: { blogSlug: string; isDraft?: boolean }) {
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2 rounded-xl border border-dashed border-amber-300 bg-amber-50 dark:bg-amber-900/30 px-3 py-2 text-xs">
      ...
      <Link to="/blogs" className="rounded-lg bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1.5 font-medium text-neutral-700 dark:text-neutral-300">
        {t('blogs.ownerToolbar.myBlogs', 'My Blogs')}
      </Link>
      <Link to="/blogs/$slug/manage" params={{ slug: blogSlug }} className="rounded-lg bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1.5 font-medium text-neutral-700 dark:text-neutral-300">
