...
export async function serveAd(
  ...
  const { rows } = await orm.execute<CandidateRow & Record<string, unknown>>(sql`
    SELECT cr.id AS creative_id, cr.campaign_id, cr.format, cr.size, cr.title, cr.body,
     ...
     WHERE cr.placement_key = ${placementKey}
       ...
       AND (c.target_plans IS NULL OR ${plan} = ANY (c.target_plans))
     ORDER BY random()
     LIMIT 1
  `);
  ...
  for (const event of events.slice(0, 20)) {
    try {
      await orm.transaction(async (tx) => {
        const { rows: creativeRows } = await tx.execute<{ campaign_id: string; cpm_credits: string; total_budget_credits: string; spent_credits: string }>(sql`
          SELECT cr.campaign_id, c.cpm_credits, c.total_budget_credits, c.spent_credits
           ...
           WHERE cr.id = ${event.creativeId} FOR UPDATE OF c
        `);
        ...
        const { rows: inserted } = await tx.execute<{ id: string }>(sql`
          INSERT INTO ad_events (creative_id, campaign_id, placement_key, user_id, event_type, cost_credits, client_event_id)
           VALUES (${event.creativeId},${row.campaign_id},${event.placementKey},${userId},${event.type},${costStr},${event.clientEventId ?? null})
        ...
        if (event.type === "impression") {
          await tx.execute(sql`
            UPDATE ad_campaigns SET spent_credits = spent_credits + ${costStr},
                    status = CASE WHEN spent_credits + ${costStr} >= total_budget_credits THEN 'completed' ELSE status END,
                    updated_at = NOW()
             WHERE id = ${row.campaign_id}
          `);
          await tx.execute(sql`UPDATE ad_creatives SET impressions_count = impressions_count + 1 WHERE id = ${event.creativeId}`);
        } else {
          await tx.execute(sql`UPDATE ad_creatives SET clicks_count = clicks_count + 1 WHERE id = ${event.creativeId}`);
        }

        await tx.execute(sql`
          INSERT INTO ad_campaign_daily_stats (campaign_id, date, impressions, clicks, spend_credits)
           VALUES (${row.campaign_id}, CURRENT_DATE, ${event.type === "impression" ? 1 : 0}, ${event.type === "click" ? 1 : 0}, ${costStr})
           ON CONFLICT (campaign_id, date) DO UPDATE
           SET impressions = ad_campaign_daily_stats.impressions + ${event.type === "impression" ? 1 : 0},
               clicks = ad_campaign_daily_stats.clicks + ${event.type === "click" ? 1 : 0},
               spend_credits = ad_campaign_daily_stats.spend_credits + ${costStr}
        `);
      });
    } catch (err) {
