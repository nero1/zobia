...
/**
 * lib/hooks/useNewMemberQuestDismissal.ts
 *
 * Client-owned dismissal state for the Home Dashboard's New Member Quest
 * card. Fast path lives in localStorage, scoped per-user-id (so a shared
 * device doesn't leak one user's dismissal into another's session) under
 * key `zobia:nmq:<userId>`. The server (POST /api/quests/new-member/dismiss)
 * is only informed occasionally — after the 4th dismissal, or immediately
 * on an explicit "don't remind me again" — per the backend route's own doc
 * comment, to keep DB writes low.
 */
...
export function useNewMemberQuestDismissal(userId: string | null | undefined) {
  ...
  const syncServer = useCallback((dismissCount: number, dontRemindAgain: boolean) => {
    fetch("/api/quests/new-member/dismiss", {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ dismissCount, dontRemindAgain }),
    }).catch(() => {
