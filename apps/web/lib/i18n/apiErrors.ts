 ...
 *
 * Key convention: `errors.<code lowercased>` — matches the existing
 * `errors.global_rate_limit` / `errors.not_room_member` keys already present
 * in every locale file, rather than mirroring the API's upper-snake-case
 * `code` field literally.
 *
 * Call from client components with the `t` function from `useTranslation()`:
 *
 *   const { t } = useTranslation();
 *   const message = translateApiError(t, error.code, error.message);
 */
