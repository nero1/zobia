...
async function awardCreditsCapped(userId: string, amount: number, referenceId: string, description: string, dailyCapCredits: number): Promise<void> {
  ...
  try {
    ...
    const { rows } = await orm.execute<{ earned: string }>(sql`
      ...
      FROM coin_ledger
      WHERE user_id = ${userId} AND transaction_type LIKE 'quiz_%' AND amount > 0
...
export async function createQuiz(input: CreateQuizInput): Promise<QuizSummary> {
  ...
  await orm.transaction(async (tx) => {
    await tx.execute(sql`
      ...
      VALUES (${quizId}, ${input.userId}, ${slug}, ${input.title.trim()}, ${input.description?.trim() || null}, ${passingScorePercent}, ${maxAttemptsPerUser})
    `);
    for (let qi = 0; qi < input.questions.length; qi++) {
      const q = input.questions[qi];
      const { rows: qRows } = await tx.execute<{ id: string }>(sql`
        INSERT INTO quiz_questions (quiz_id, position, prompt, type, points) VALUES (${quizId}, ${qi}, ${q.prompt.trim()}, ${q.type}, ${q.points && q.points > 0 ? q.points : 1}) RETURNING id
      `);
...
export async function getQuizBySlug(slug: string, viewerId?: string | null, includeAnswers = false): Promise<QuizDetail | null> {
  const orm = await getDb();
  const { rows } = await orm.execute<QuizRow & Record<string, unknown>>(sql`
    ...
    WHERE q.slug = ${slug} AND q.deleted_at IS NULL LIMIT 1
  `);
  const quiz = rows[0];
  if (!quiz) return null;
  ...
  const { rows: questionRows } = await orm.execute<{ id: string; prompt: string; type: string; points: number }>(
    sql`SELECT id, prompt, type, points FROM quiz_questions WHERE quiz_id = ${quiz.id} ORDER BY position ASC`
  );
  const { rows: optionRows } = await orm.execute<{ id: string; question_id: string; label: string; is_correct: boolean }>(sql`
    SELECT o.id, o.question_id, o.label, o.is_correct FROM quiz_question_options o
    JOIN quiz_questions q ON q.id = o.question_id WHERE q.quiz_id = ${quiz.id} ORDER BY o.position ASC
  `);
  ...
  if (viewerId) {
    const { rows: attemptRows } = await orm.execute<{ score: number; total_points: number; score_percent: number; passed: boolean }>(
      sql`SELECT score, total_points, score_percent, passed FROM quiz_attempts WHERE quiz_id = ${quiz.id} AND user_id = ${viewerId} ORDER BY score_percent DESC LIMIT 1`
    );
    const { rows: countRows } = await orm.execute<{ count: string }>(sql`SELECT COUNT(*)::text AS count FROM quiz_attempts WHERE quiz_id = ${quiz.id} AND user_id = ${viewerId}`);
...
export async function recordQuizView(quizId: string): Promise<void> {
  const orm = await getDb();
  await orm.execute(sql`UPDATE quizzes SET view_count = view_count + 1 WHERE id = ${quizId}`).catch(() => {});
}
...
export async function listQuizzes(tab: "new" | "popular" | "mine", cursor: string | undefined, limit: number, viewerId?: string | null): Promise<ListQuizzesResult> {
  let where = sql`q.status = 'active' AND q.deleted_at IS NULL`;
  if (tab === "mine") {
    if (!viewerId) throw forbidden("Sign in to view your quizzes.");
    where = sql`q.creator_id = ${viewerId} AND q.deleted_at IS NULL`;
  }
  if (cursor) {
    where = sql`${where} AND q.created_at < (SELECT created_at FROM quizzes WHERE id = ${cursor})`;
  }
  const orderBy = tab === "popular" ? sql`q.attempt_count DESC, q.created_at DESC` : sql`q.created_at DESC`;

  const orm = await getDb();
  const { rows } = await orm.execute<{ id: string; slug: string; title: string; attempt_count: number; created_at: string; creator_username: string | null }>(sql`
...
export async function submitQuizAttempt(input: SubmitAttemptInput): Promise<SubmitAttemptResult> {
  ...
  const orm = await getDb();
  const { rows: quizRows } = await orm.execute<{ id: string; status: string; passing_score_percent: number; max_attempts_per_user: number }>(
    sql`SELECT id, status, passing_score_percent, max_attempts_per_user FROM quizzes WHERE id = ${input.quizId} AND deleted_at IS NULL LIMIT 1`
  );
  const quiz = quizRows[0];
  if (!quiz) throw notFound("Quiz not found");
  if (quiz.status !== "active") throw badRequest("This quiz is no longer accepting attempts.", "QUIZ_NOT_ACTIVE");

  const { rows: countRows } = await orm.execute<{ count: string }>(sql`SELECT COUNT(*)::text AS count FROM quiz_attempts WHERE quiz_id = ${input.quizId} AND user_id = ${input.userId}`);
  ...
  const { rows: questionRows } = await orm.execute<{ id: string; type: string; points: number }>(sql`SELECT id, type, points FROM quiz_questions WHERE quiz_id = ${input.quizId}`);
  if (questionRows.length === 0) throw badRequest("This quiz has no questions.", "QUIZ_NO_QUESTIONS");
  const { rows: optionRows } = await orm.execute<{ id: string; question_id: string; is_correct: boolean }>(sql`
    SELECT o.id, o.question_id, o.is_correct FROM quiz_question_options o
    JOIN quiz_questions q ON q.id = o.question_id WHERE q.quiz_id = ${input.quizId}
  `);
  ...
  await orm.transaction(async (tx) => {
    const { rows: attemptRows } = await tx.execute<{ id: string }>(sql`
      INSERT INTO quiz_attempts (quiz_id, user_id, attempt_number, score, total_points, score_percent, passed, completed_at)
      VALUES (${input.quizId}, ${input.userId}, ${attemptsSoFar + 1}, ${score}, ${totalPoints}, ${scorePercent}, ${passed}, NOW()) RETURNING id
    `);
    const attemptId = attemptRows[0].id;
    for (const a of gradedAnswers) {
      await tx.execute(sql`INSERT INTO quiz_attempt_answers (attempt_id, question_id, selected_option_ids, is_correct) VALUES (${attemptId}, ${a.questionId}, ${JSON.stringify(a.selectedOptionIds)}, ${a.isCorrect})`);
    }
    await tx.execute(sql`UPDATE quizzes SET attempt_count = attempt_count + 1 WHERE id = ${input.quizId}`);
  });
...
export async function shareQuiz(userId: string, quizId: string): Promise<{ shareCount: number; rewardClaimed: number | null }> {
  await requireFeatureEnabled("quizzes");
  const orm = await getDb();
  const { rows } = await orm.execute<{ id: string }>(sql`SELECT id FROM quizzes WHERE id = ${quizId} AND deleted_at IS NULL LIMIT 1`);
  ...
  const { rewardClaimed } = await recordContentShare("quiz", quizId, userId, "quiz_treasury_claim", manifest.features.quizMonetization, async (tx) => {
    await tx.execute(sql`UPDATE quizzes SET share_count = share_count + 1 WHERE id = ${quizId}`);
  });

  const { rows: countRows } = await orm.execute<{ share_count: number }>(sql`SELECT share_count FROM quizzes WHERE id = ${quizId}`);
...
export async function fundQuizTreasury(userId: string, quizId: string, amount: number, maxClaimants: number): Promise<TreasuryState> {
  ...
  const orm = await getDb();
  const { rows } = await orm.execute<{ creator_id: string }>(sql`SELECT creator_id FROM quizzes WHERE id = ${quizId} AND deleted_at IS NULL LIMIT 1`);
  const quiz = rows[0];
  if (!quiz) throw notFound("Quiz not found");
...
export async function getQuizIdBySlug(slug: string): Promise<string | null> {
  const orm = await getDb();
  const { rows } = await orm.execute<{ id: string }>(sql`SELECT id FROM quizzes WHERE slug = ${slug} AND deleted_at IS NULL LIMIT 1`);
...
export async function setQuizStatus(quizId: string, status: "active" | "closed" | "disabled"): Promise<void> {
  const orm = await getDb();
  const result = await orm.execute(sql`UPDATE quizzes SET status = ${status}, updated_at = NOW() WHERE id = ${quizId} AND deleted_at IS NULL`);
  if (!result.rowCount) throw notFound("Quiz not found");
}

export async function deleteQuiz(quizId: string): Promise<void> {
  const orm = await getDb();
  const result = await orm.execute(sql`UPDATE quizzes SET deleted_at = NOW() WHERE id = ${quizId} AND deleted_at IS NULL`);
  if (!result.rowCount) throw notFound("Quiz not found");
}
