...
export const GET = async (req: NextRequest) => {
  ...
  const didClaim = await checkCronIdempotency("cron_daily_social_last_run", orm);
  if (!didClaim) {
  ...
  try {
    if (isSunday) {
      results.nemesisRefresh = await refreshNemesisAssignments(orm);
    } else {
  ...
  try {
    results.nemesisChallengeExpiry = await expireUnacceptedNemesisChallenges(orm);
  } catch (err) {
  ...
  try {
    if (isSunday) {
      const { rows: activeSeasons } = await orm.execute<{ id: string; name: string; starts_at: string }>(sql`
      ...
      if (activeSeasons[0]) {
        ...
        await orm.execute(sql`DELETE FROM leaderboard_rank_snapshots WHERE scope = ${seasonScope}`);
        await orm.execute(sql`
          INSERT INTO leaderboard_rank_snapshots (user_id, scope, rank, xp, snapped_at)
          SELECT ls.user_id, ${seasonScope},
                 ROW_NUMBER() OVER (ORDER BY ls.xp_value DESC) AS rank,
                 ls.xp_value, NOW()
          FROM leaderboard_snapshots ls
          JOIN users u ON u.id = ls.user_id
          WHERE ls.season_id = ${season.id} AND ls.scope = 'season' AND u.deleted_at IS NULL
        ...
        if (weekNum >= 6) {
          await orm.execute(sql`
            ...
            SELECT ls.user_id, 'season_top100_frame', 'season_top100_frame:s' || ${season.id}::text, ${season.id}, NOW()
            FROM (
  ...
  try {
    const { rows: currentRanks } = await orm.execute<{ user_id: string; rank: string; xp_value: string }>(sql`
  ...
  try {
    const milestoneValues = sql.raw(STICKER_MILESTONES.map(m => `(${m})`).join(','));
    const { rows: newMilestones } = await orm.execute<{
      user_id_a: string;
      user_id_b: string;
      milestone_score: number;
      pack_id: string | null;
    }>(sql`
      WITH milestones(m) AS (VALUES ${milestoneValues}),
      eligible AS (
    ...
    if (newMilestones.length > 0) {
      ...
      const uaIds = newMilestones.map(r => r.user_id_a);
      const ubIds = newMilestones.map(r => r.user_id_b);
      const milestoneScores = newMilestones.map(r => r.milestone_score);
      await orm.execute(sql`
        INSERT INTO notifications (user_id, type, title, body, metadata, is_read, created_at)
  ...
  try {
    const { rows: staleUsers } = await orm.execute<{ id: string }>(sql`
    ...
    if (staleUsers.length > 0) {
      await batchCalculateTrustScores(staleUsers.map(u => u.id), orm);
    }
  ...
  // 6. Earnable sticker pack auto-unlock — fully set-based
  try {
    const { rows: newUnlocks } = await orm.execute<{ user_id: string; pack_id: string; pack_name: string; track: string; level: number }>(sql`
      WITH new_grants AS (
        INSERT INTO user_sticker_packs (user_id, pack_id, unlocked_at)
  ...
  // 7. Creator tier progression — single UPDATE...FROM CTE (was per-creator loop)
  try {
    const { rowCount: tierUpdates } = await orm.execute(sql`
      UPDATE users u
  ...
  try {
    if (isSunday) {
      const { rows: overtakeRows } = await orm.execute<{ user_id: string; nemesis_user_id: string; user_xp: number; nemesis_xp: number }>(sql`
      ...
      if (overtakeRows.length > 0) {
        ...
        const ouIds = overtakeRows.map(r => r.user_id);
        const onIds = overtakeRows.map(r => r.nemesis_user_id);
        const ouXps = overtakeRows.map(r => r.user_xp);
        const onXps = overtakeRows.map(r => r.nemesis_xp);
        await orm.execute(sql`
          INSERT INTO notifications (user_id, type, title, body, metadata, is_read, created_at)
