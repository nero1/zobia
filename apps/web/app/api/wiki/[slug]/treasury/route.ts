 ...
 * POST   /api/wiki/<slug>/treasury  — create the pot (owner only, Credits;
 *   only when none already exists)
 * PATCH  /api/wiki/<slug>/treasury  — edit an existing pot's amount/max
 *   claimants; debits an increase or refunds a decrease
 * DELETE /api/wiki/<slug>/treasury  — turn the pot off, refunding unclaimed
 *   funds to the owner
 *
...
import { fundWikiTreasury, editWikiTreasury, closeWikiTreasury, getWikiTreasury } from "@/lib/wiki/service";
...
export const PATCH = withAuth<{ slug: string }>(async (req: NextRequest, { params, auth }) => {
  try {
    ...
    if (!wiki) throw notFound("Wiki not found");
    const body = await validateBody(req, fundSchema);
    const treasury = await editWikiTreasury(wiki.id, auth.user.sub, body.amount, body.maxClaimants);
    return NextResponse.json({ success: true, data: { treasury }, error: null });
  } catch (err) {
    return handleApiError(err);
  }
...
export const DELETE = withAuth<{ slug: string }>(async (req: NextRequest, { params, auth }) => {
  try {
    ...
    const wiki = await getWikiBySlug(params.slug);
    if (!wiki) throw notFound("Wiki not found");
    const treasury = await closeWikiTreasury(wiki.id, auth.user.sub);
    return NextResponse.json({ success: true, data: { treasury }, error: null });
  } catch (err) {
    return handleApiError(err);
  }
