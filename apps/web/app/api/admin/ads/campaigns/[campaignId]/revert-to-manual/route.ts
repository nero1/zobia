...
export const POST = withAdminAuth(async (req: NextRequest, { params, auth }: Ctx) => {
  try {
    ...
    const { rows } = await db.query<{ id: string; name: string; created_by: string; moderation_status: string; moderation_mode: string | null }>(
      `SELECT id, name, created_by, moderation_status, moderation_mode FROM ad_campaigns WHERE id = $1 AND deleted_at IS NULL LIMIT 1`,
      [campaignId]
    );
    ...
    await db.query(
      `UPDATE ad_campaigns
       SET status = 'pending_review', moderation_status = 'pending', moderation_reason = $1, updated_at = NOW()
       WHERE id = $2`,
      [body.reason ?? "Reverted from AI auto-approval for manual re-review.", campaignId]
    );
    ...
    await db
      .query(
        ...
        [
          campaign.created_by,
          "Ad campaign under re-review",
          `Your ad campaign "${campaign.name}" was pulled back for a manual review.${body.reason ? ` Reason: ${body.reason}` : ""}`,
          JSON.stringify({ campaignId, moderationStatus: "pending" }),
        ]
      )
    ...
    await db
      .query(
        `INSERT INTO admin_audit_log (admin_id, action, resource, resource_id, after_val, created_at)
         VALUES ($1, 'ad_campaign_ai_decision_reverted', 'ad_campaign', $2, $3::jsonb, NOW())`,
        [auth.user.sub, campaignId, JSON.stringify({ reason: body.reason ?? null })]
      )
