...
/**
 * app/api/admin/wiki/[id]/status/route.ts
 *
 * PATCH /api/admin/wiki/<id>/status — moderator/admin action on a wiki:
 *   { action: "suspend" | "ban" | "deactivate" | "pause" | "restore" | "delete", reason?: string }
 * Every action is recorded in wiki_moderation_log.
 */
...
import { setWikiStatus } from "@/lib/wiki/service";
...
export const PATCH = withModeratorOrAdminAuth<{ id: string }>(async (req: NextRequest, { params, auth }) => {
  try {
    ...
    await setWikiStatus(params.id, auth.user.sub, body.action, body.reason);
