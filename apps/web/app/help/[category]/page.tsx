    ...
    return (
      <main id="main-content" className="min-h-screen bg-background">
        <div className="max-w-3xl mx-auto px-4 py-12">
          <Link href="/help" className="text-sm text-primary underline">&larr; {t("help.homeTitle")}</Link>
          ...
          {byDifficulty.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("help.noDocsPublished")}</p>
          ) : (
            <div className="space-y-8">
              {byDifficulty.map((group) => (
                <section key={group.difficulty}>
                  <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">{t(`help.difficulty.${group.difficulty}`)}</h2>
