 ...
 *
 * Polls — discovery page. Tabs: New / Popular / Mine. Cursor pagination via
 * "Load More" (mirrors app/(app)/answers/page.tsx's state machine).
 */
...
function PollCard({ p }: { p: PollSummary }) {
  ...
  return (
    ...
    >
      <h3 className="line-clamp-2 text-sm font-semibold text-neutral-900 dark:text-neutral-50">📊 {p.title}</h3>
      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-neutral-500 dark:text-neutral-400">
        ...
        <span>·</span>
        <span>{p.voterCount} {p.voterCount === 1 ? t("polls.list.voter", "voter") : t("polls.list.voters", "voters")}</span>
      </div>
    </Link>
  );
}
...
export default function PollsPage() {
  ...
  const fetchPolls = useCallback(async (nextTab: Tab, append = false, afterCursor: string | null = null) => {
    if (append) setLoadingMore(true); else setLoading(true);
    setError(null);
    try {
      const params = new URLSearchParams({ tab: nextTab, limit: "20" });
      if (afterCursor) params.set("cursor", afterCursor);
      const res = await fetch(`/api/polls?${params.toString()}`, { credentials: "include" });
      ...
      if (!res.ok) {
        ...
        throw new Error(json?.error?.message ?? "Failed to load polls");
      }
      setFeatureDisabled(false);
      const data = json.data as { polls: PollSummary[]; nextCursor: string | null };
  ...
  useEffect(() => {
    setCursor(null);
    void fetchPolls(tab, false, null);
  ...
  return (
    <NotFoundGate enabled={flags.polls ?? true} isAdmin={isAdmin} featureLabel={t("polls.title", "Polls")}>
      <div className="mx-auto max-w-2xl p-4 sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h1 className="text-2xl font-bold text-neutral-900 dark:text-neutral-50">{t("polls.title", "Polls")}</h1>
          <Link href="/polls/new" className="rounded-xl bg-primary-600 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-700">
            {t("polls.createCta", "Create Poll")}
          </Link>
        </div>
        ...
        {featureDisabled ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-neutral-200 bg-white py-16 text-center dark:border-neutral-800 dark:bg-neutral-900">
            ...
            <p className="mt-3 text-sm font-semibold text-neutral-700 dark:text-neutral-300">
              {t("polls.disabled", "Polls are currently disabled.")}
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="space-y-3">
              ...
              {hasMore && !loading && (
                <button
                  onClick={() => void fetchPolls(tab, true, cursor)}
                ...
                >
                  {loadingMore ? t("polls.loadingMore", "Loading…") : t("polls.loadMore", "Load More")}
                </button>
              )}
            </div>
          </>
