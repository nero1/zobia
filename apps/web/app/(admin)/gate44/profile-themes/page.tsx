...
/**
 * app/(admin)/gate44/profile-themes/page.tsx
 *
 * Admin control over the profile theme catalog (migration 0050): toggle
 * enabled, plan/business-tier gating, and credits/stars price per theme.
 * Mirrors app/(admin)/gate44/blogs/themes/page.tsx exactly, minus the
 * layout-variant concept (profile themes are color skins only).
 */
...
export default function AdminProfileThemesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/profile-themes", { credentials: "include" });
  ...
  async function patch(id: string, body: Record<string, unknown>) {
    setBusy(id);
    try {
      const res = await fetch(`/api/admin/profile-themes/${id}`, {
        method: "PATCH",
        credentials: "include",
  ...
  return (
    <div className="relative">
      <h1 className="mb-2 text-2xl font-bold text-neutral-900 dark:text-neutral-50">Profile Themes</h1>
      <p className="mb-6 text-sm text-neutral-500">
        One free-default theme is always available to every profile. Everything else is gated by plan/business tier, or purchasable with credits/stars via the existing cosmetics ledger — sold on the Market page under Cosmetics & Themes.
      </p>
