...
/**
 * app/(admin)/gate44/forum/queue/page.tsx
 *
 * Forum moderation queue — mirrors app/(admin)/gate44/answers/queue/page.tsx,
 * scoped to reports targeting bbforum threads/posts. Accessible to admins
 * and moderators.
 */
...
function ReportCard({ report, onAction, busy, isAdmin }: { report: BbforumReport; onAction: (id: string, action: string) => void; busy: string | null; isAdmin: boolean }) {
  const isBusy = busy === report.id;
  const targetLabel = report.reported_bb_thread_id ? "thread" : "post";
  const preview = report.thread_title ?? report.post_body ?? "(content unavailable)";
  const postHref = report.thread_slug ? `/f/${report.thread_slug}` : null;
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white p-4 shadow-card dark:border-neutral-800 dark:bg-neutral-900">
      <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
        ...
        {postHref && <a href={postHref} target="_blank" rel="noopener noreferrer" title="Open post" className="text-neutral-400 hover:text-primary-600 dark:hover:text-primary-400">🔗</a>}
        {report.content_author_id && (
          <a href={`/profile/${report.content_author_id}`} target="_blank" rel="noopener noreferrer" className="text-neutral-500 hover:text-primary-600 hover:underline dark:hover:text-primary-400">author</a>
        )}
        <span className="rounded-full bg-neutral-100 px-2 py-0.5 font-semibold text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400">{report.report_type.replace(/_/g, " ")}</span>
      ...
      {report.status === "pending" && (
        <div className="flex flex-wrap gap-1.5">
          {actions.filter((a) => !a.hidden).map(({ label, action, classes }) => (
            <button key={action} disabled={isBusy} onClick={() => onAction(report.id, action)} className={`flex items-center justify-center rounded-lg px-2.5 py-1 text-xs font-semibold transition-colors disabled:opacity-50 ${classes}`}>
...
export default function ForumQueuePage() {
  const [tab, setTab] = useState<TabKey>("pending");
  const [reports, setReports] = useState<BbforumReport[]>([]);
  ...
  const fetchReports = useCallback(async (status: TabKey) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/bbforum/queue?status=${status}`, { credentials: "include" });
      if (res.status === 401 || res.status === 403) { window.location.href = "/gate44/login"; return; }
      if (!res.ok) throw new Error("Failed to load reports");
      const data = (await res.json()) as { data?: { items?: BbforumReport[] } };
  ...
  async function handleAction(reportId: string, action: string) {
    setBusy(reportId);
    try {
      const res = await fetch(`/api/admin/bbforum/queue/${reportId}/action`, {
        method: "POST",
        credentials: "include",
  ...
  return (
    <div className="relative">
      ...
      <div className="mb-6 flex gap-1 rounded-xl border border-neutral-200 bg-neutral-100 p-1 dark:border-neutral-800 dark:bg-neutral-800/50">
        {tabs.map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)} className={`flex-1 rounded-lg py-2 text-sm font-semibold transition-colors ${tab === key ? "bg-white text-neutral-900 shadow-card dark:bg-neutral-900 dark:text-neutral-50" : "text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300"}`}>
            {label}
          </button>
        ))}
      </div>

      {error && <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-800 dark:bg-red-950 dark:text-red-300">{error}</div>}
