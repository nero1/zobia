...
export default function AdminGamesPage() {
  ...
  async function viewStats(g: GameRow) {
    ...
    const b = await res.json();
    if (b?.data) setStats(b.data as GameStats);
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      {/* List view (table) — columns collapse on narrow screens, and the
          remaining table still scrolls horizontally within its own bordered
          wrapper (not the page) with visible momentum scrolling on iOS. */}
      {viewMode === "list" && (
        <div className="overflow-x-auto rounded-xl border border-neutral-200 dark:border-neutral-800" style={{ WebkitOverflowScrolling: "touch" }}>
        <table className="w-full min-w-[520px] text-sm">
          <thead>
            <tr className="border-b text-left text-neutral-500">
              <th className="py-2 pl-3">Game</th>
              <th className="hidden sm:table-cell">Category</th>
              <th className="hidden sm:table-cell">Plays</th>
              <th>Players</th>
          ...
          <tbody>
            {filtered.map((g) => (
              <tr key={g.id} className="border-b hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                <td className="py-2 pl-3">{g.cover_emoji} {g.name} <span className="text-xs text-neutral-400">/{g.slug}</span></td>
                <td className="hidden sm:table-cell"><span className="rounded-full bg-neutral-100 px-2 py-0.5 text-xs dark:bg-neutral-800">{g.category}</span></td>
                <td className="hidden sm:table-cell">{g.play_count}</td>
                <td>{g.players}</td>
                <td className="hidden md:table-cell text-xs text-neutral-600 dark:text-neutral-400">{g.reward_credits_per_win}c/{g.reward_xp_per_win}xp{g.reward_stars_per_win ? `/${g.reward_stars_per_win}⭐` : ""}</td>
                <td className="hidden md:table-cell text-xs">{g.play_cost_credits || g.play_cost_stars ? `${g.play_cost_credits}c${g.play_cost_stars ? `/${g.play_cost_stars}⭐` : ""}` : <span className="text-emerald-600">Free</span>}</td>
                <td>
                ...
                <td className="space-x-2 whitespace-nowrap pr-3 text-right">
...
// ---------------------------------------------------------------------------
// Stats modal — formatted stat tiles instead of a raw JSON dump
// ---------------------------------------------------------------------------
...
function StatTile({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-xl border border-neutral-200 bg-white p-3 dark:border-neutral-700 dark:bg-neutral-900">
      ...
      <p className="mt-0.5 text-lg font-bold text-neutral-900 dark:text-neutral-100">{value}</p>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-full max-w-md rounded-2xl bg-neutral-50 p-5 shadow-xl dark:bg-neutral-800">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <StatTile label="Unique Players" value={stats.plays.unique_players.toLocaleString()} />
          <StatTile label="Completed Plays" value={stats.plays.counted_plays.toLocaleString()} />
          <StatTile label="Wins Rewarded" value={stats.winsRewarded.toLocaleString()} />
          <StatTile label="Avg Score" value={stats.plays.avg_score !== null ? Math.round(stats.plays.avg_score).toLocaleString() : "—"} />
          <StatTile label="Max Score" value={stats.plays.max_score !== null ? stats.plays.max_score.toLocaleString() : "—"} />
          <StatTile label="Challenges" value={`${stats.challenges.completed}/${stats.challenges.total}`} />
          <StatTile label="Wager Volume" value={stats.challenges.wager_volume.toLocaleString()} />
        </div>
      </div>
    </div>
  );
