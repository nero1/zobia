...
import { getAllPlanOptions, getPersonalPlanValues } from "@/lib/plans/allPlanOptions";
...
// Sourced from the platform's canonical plan/business-tier constants (see
// lib/plans/allPlanOptions.ts) instead of a hardcoded copy — a plan/tier
// added or removed there shows up here automatically. Previously this was
...
function AnnForm({ type, initial, onSave, onCancel }: AnnFormProps) {
  ...
  // New announcements default to every non-business plan + every role selected;
  // editing an existing one keeps exactly what was saved (even if empty = "all").
  const [selectedPlans, setSelectedPlans] = useState<string[]>(
    initial?.audience?.plans ?? DEFAULT_SELECTED_PLANS
  );
            ...
            {PLAN_OPTIONS.map((p) => (
              <label key={p.value} className="flex cursor-pointer items-center gap-1.5 text-sm">
                <input
                  ...
                  onChange={() => setSelectedPlans(toggleArr(selectedPlans, p.value))}
  ...
  async function handleSaveDisplayMode(mode: DisplayMode) {
    try {
      await fetch("/api/admin/announcements/display-mode", {
        ...
        body: JSON.stringify({ mode, type: annType }),
      });
      setDisplayMode(mode);
