...
/**
 * app/(admin)/gate44/settings/referrals/page.tsx
 *
 * Admin panel — Referral Stats settings.
 *
 * Controls which plans/prestige tiers get the "Full" referral stats view
 * (daily visit breakdown, top pages, conversion rate, and the per-referral
 * list) on /referrals. Everyone else gets the "Basic" view: just totals
 * (referral counts, coins/XP earned, all-time link visits). Stored in
 * x_manifest and read back via GET /api/admin/config — same pattern as
 * Profile Stats (key: profile_stats_full_plans).
 */
...
export default function AdminReferralStatsSettingsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setFullPlans(parseJsonList(get("referral_stats_full_plans"), defaultFullPlans()));
      } catch { /* non-fatal */ }
  ...
  async function save() {
    setSaving(true);
    try {
      const res = await fetch("/api/admin/config/referral_stats_full_plans", {
        method: "PUT",
        credentials: "include",
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 p-4 sm:p-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-neutral-900 dark:text-neutral-50">Referral Stats Settings</h1>
        <p className="mt-1 text-sm text-neutral-500">
          Control which plans and prestige ranks get the Full referral stats view: daily link-visit
          breakdown, top pages, conversion rate, and the per-referral list. Everyone else sees the Basic
          view — totals only (referral counts, coins/XP earned, all-time link visits). Changes take effect
          within 60 seconds.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
        <div className="border-b border-neutral-200 px-5 py-4 dark:border-neutral-800">
          <h2 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">Who gets the Full referral stats view</h2>
          <p className="mt-0.5 text-xs text-neutral-500">
            Applies to GET /api/referrals (web, PWA, and the Capacitor Android app all read this one endpoint).
          </p>
        </div>
        <div className="p-5">
          <div className="mb-4">
