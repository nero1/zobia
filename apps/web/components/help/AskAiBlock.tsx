...
export function AskAiBlock({ docId, docTitle }: { docId: string; docTitle: string }) {
  ...
  async function ask() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.error?.message ?? "The AI assistant is unavailable right now.");
  ...
  if (loggedIn === false) {
    return (
      <div className="rounded-xl border border-border bg-muted/30 p-5 text-center">
        <p className="mb-3 text-sm text-muted-foreground">Can&apos;t find what you&apos;re looking for? Log in or sign up to ask the AI assistant or contact a support staff member.</p>
        <div className="flex justify-center gap-3">
          <a href="/auth/login" className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Log in</a>
          <a href="/auth/login?signup=1" className="rounded-lg border border-border px-4 py-2 text-sm font-semibold">Sign up</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/30 p-5">
      ...
      <p className="mb-3 text-xs text-muted-foreground">Can&apos;t find what you&apos;re looking for? Try asking the AI.</p>
      ...
      {!answer ? (
        ...
        <div>
            ...
            <div>
              {eligibility && !eligibility.freeAccess && !eligibility.helpCenterFree && !eligibility.blocked && (
                <p className="mb-2 text-xs text-muted-foreground">
                  Contacting a person costs {eligibility.costCredits > 0 && `${eligibility.costCredits} credits`}
                  {eligibility.costCredits > 0 && eligibility.costStars > 0 && " or "}
                  {eligibility.costStars > 0 && `${eligibility.costStars} stars`}. Response time is usually 24-48 hours.
                </p>
              )}
              {(!eligibility || eligibility.freeAccess || eligibility.helpCenterFree) && (
                <p className="mb-2 text-xs text-muted-foreground">Response time is usually 24-48 hours or less.</p>
              )}
          ...
          <button onClick={() => { setAnswer(null); setQuestion(""); }} className="ml-2 mt-2 text-xs text-muted-foreground underline">Ask another question</button>
        </div>
      )}
