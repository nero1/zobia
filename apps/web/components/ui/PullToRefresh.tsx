 ...
 *
 * Small, dependency-free touch pull-to-refresh wrapper for mobile web/PWA
 * (also rendered inside the Capacitor WebView mirror). Tracks touchstart/
 * touchmove Y-delta only when the page is already scrolled to the top
 * (window.scrollY === 0), shows a simple arrow/spinner indicator past a
 * threshold, and calls `onRefresh` on release past that threshold.
 *
...
export function PullToRefresh({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex h-7 w-7 items-center justify-center rounded-full border-2 border-neutral-300 text-neutral-500 dark:border-neutral-700 dark:text-neutral-400 ${
            refreshing ? "animate-spin border-t-blue-500" : ""
          }`}
