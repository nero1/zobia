 ...
 *
 * Shared admin user search/list/detail component, extracted out of
 * app/(admin)/gate44/users/page.tsx so it can be reused by both:
 *   - app/(admin)/gate44/users/page.tsx (unchanged behavior — thin wrapper)
 *   - app/(admin)/gate44/data-management/page.tsx (Users tab — same
 *     search/view/impersonate/suspend/ban/etc, plus a selection checkbox
 *     column and a "Delete" action when `embedded` is true).
 *
 * Search by username, email, or ID. Click a row to open a detail panel with
 * actions (suspend/ban/restore/mod toggles/reset password/force 2FA/verify,
 * impersonate, and — when embedded — soft-delete).
 */
...
export const PLAN_BADGE: Record<Plan, { label: string; classes: string }> = {
...
export const STATUS_BADGE: Record<UserStatus, { label: string; classes: string }> = {
...
function RowSkeleton({ selectable }: { selectable?: boolean }) {
  return (
    <tr>
      {Array.from({ length: selectable ? 7 : 6 }).map((_, i) => (
...
const ACTION_LABELS: Record<ActionType, string> = {
  ...
  upgrade_support: "make support staff",
  ...
  delete_account: "permanently delete (soft-delete)",
};

function DetailPanel({ user, onClose, onAction, onImpersonate, onDelete, showDelete }: DetailPanelProps) {
  ...
  return (
    <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-sm flex-col overflow-y-auto border-l border-neutral-200 bg-white shadow-modal dark:border-neutral-800 dark:bg-neutral-900 sm:w-96">
      ...
      <div className="flex-1 space-y-5 p-4">
        ...
        {showDelete && onDelete && (
          <div className="space-y-2 rounded-lg border border-danger-200 bg-danger-50 p-3 dark:border-danger-800 dark:bg-danger-950/30">
            <p className="text-xs font-semibold uppercase tracking-wider text-danger-700 dark:text-danger-300">Danger Zone</p>
            <p className="text-[11px] text-danger-600 dark:text-danger-400">
              Soft-deletes and anonymizes this account (same as a self-service account deletion). This cannot be undone from the UI.
            </p>
            <button
              onClick={() => setConfirmDelete(true)}
              disabled={deleting}
              className="flex min-h-[44px] w-full items-center justify-center rounded-lg bg-danger-600 px-3 py-3 text-xs font-semibold text-white hover:bg-danger-700 disabled:opacity-60"
            >
      ...
      {confirmDelete && (
        ...
        >
          <div className="w-full max-w-sm rounded-2xl border border-neutral-200 bg-white p-5 shadow-modal dark:border-neutral-800 dark:bg-neutral-900">
            <h3 className="text-base font-bold text-neutral-900 dark:text-neutral-50">Delete this account?</h3>
            <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
              This will soft-delete and anonymize <span className="font-semibold">@{user.username}</span>. This cannot be undone from the UI.
            </p>
            <div className="mt-4 flex justify-end gap-2">
              <button
...
export function ActionButton({ label, onClick, loading, disabled, className = "" }: ActionButtonProps) {
  return (
    <button
      onClick={onClick}
...
// Human-readable labels for the granular export field checkboxes
// (app/(admin)/gate44/data-management/page.tsx's ExportModal) — kept next to
// AdminUser/PLAN_BADGE/etc. so the export field list and this table's own
// columns stay described consistently.
export const ALLOWED_FIELD_LABELS: Record<string, string> = {
  id: "ID",
  username: "Username",
  email: "Email",
...
export interface UserManagementTableProps {
  /** True when rendered inside /gate44/data-management — enables the selection
   * checkbox column and the detail panel's "Delete Account" action. The
   * standalone /gate44/users page omits this to keep its behavior unchanged. */
  embedded?: boolean;
  /** Called with the current set of selected user IDs whenever selection changes (embedded mode only). */
  onSelectionChange?: (userIds: string[]) => void;
}

/**
 * Admin user search/list/detail table.
 * Requires admin authentication (enforced server-side by middleware).
 */
export default function UserManagementTable({ embedded = false, onSelectionChange }: UserManagementTableProps) {
  ...
  async function handleAction(
    ...
    if (action === "delete_account") return; // handled by handleDelete below
  ...
  async function handleDelete(userId: string) {
    const res = await fetch(`/api/admin/data-management/users/${userId}`, {
      method: "DELETE",
      credentials: "include",
    });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      showToast(body.error?.message ?? "Could not delete account", "error");
      return;
    }
  ...
  return (
    <div className="relative">
      ...
      {embedded && selectedIds.size > 0 && (
        <div className="mb-3 flex items-center justify-between rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-700 dark:bg-blue-950/40 dark:text-blue-300">
          <span>{selectedIds.size} user(s) selected for export</span>
          <button
      ...
      <div className="overflow-x-auto rounded-xl border border-neutral-200 bg-white shadow-card dark:border-neutral-800 dark:bg-neutral-900">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
            {loading
              ? Array.from({ length: 6 }).map((_, i) => <RowSkeleton key={i} selectable={embedded} />)
              : users.length === 0
              ? (
                <tr>
                  <td colSpan={embedded ? 7 : 6} className="py-12 text-center text-neutral-500">
              ...
              : users.map((u) => (
                ...
                >
                  ...
                  <td className="px-4 py-3" onClick={() => setSelected(u)}>
                    <div className="flex items-center gap-2">
                  ...
                  <td className="px-4 py-3 text-neutral-500" onClick={() => setSelected(u)}>{formatDate(u.joinedAt)}</td>
                  <td className="px-4 py-3" onClick={() => setSelected(u)}>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${STATUS_BADGE[u.status].classes}`}>
                      {STATUS_BADGE[u.status].label}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right" onClick={() => setSelected(u)}>
